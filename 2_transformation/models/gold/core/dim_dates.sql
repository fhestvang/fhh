...
date_dimension as (
    select
        ...
        isoyear(date_key) as iso_year,
        weekofyear(date_key) as iso_week,
        isoyear(date_key)::varchar || '-W' || lpad(weekofyear(date_key)::varchar, 2, '0') as iso_year_week,
        ...
        case
            when month(date_key) in (12, 1, 2) then 1
            when month(date_key) in (3, 4, 5) then 2
            when month(date_key) in (6, 7, 8) then 3
            else 4
        ...
        -- Business metrics
        case
            when dayofweek(date_key) not in (0, 6) and h.holiday_name is null then 1
            else 0
